...
func TestCreateTar(t *testing.T) {
	testCases := []struct {
		...
		{
			description: "Single file sub-folder",
			setupFiles: func(t *testing.T, tempDir string) ([]string, []string) {
				subPath := filepath.Join(tempDir, "sub")
				err := os.Mkdir(subPath, 0755)
				assert.NoError(t, err)

				filePath := filepath.Join(subPath, "testfile.txt")
				err = os.WriteFile(filePath, []byte("hello world"), 0644)
				assert.NoError(t, err)
				return nil, []string{tempDir}
			},
			expectedFiles: []string{"sub/testfile.txt"},
			shouldError:   false,
		},
		{
	...
	for _, testCase := range testCases {
		t.Run(testCase.description, func(t *testing.T) {
			...
			// Call CreateTar
			cpy := sources
			tarStream, err := CreateTar(excludes, cpy...)
